...
# Define US-NY holidays
us_ny_holidays = holidays.US(state='NY', years=range(2016, 2025))

# Collect valid parquet files for this year only (skip empty files)
...
# Load dask dataframe for this year
...
# Write output parquet for this year (no partitioning by year since folder is per year)
ddf.to_parquet(
    output_path,
    engine="pyarrow",
    write_index=False,
